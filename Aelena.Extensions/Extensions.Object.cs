...
namespace Aelena.SimpleExtensions.ObjectExtensions
{
    public static class Extensions
    {
        ...
        public static bool AreAllNull<T> ( this T obj, IList<string> fieldNames )
        {
            if ( obj == null )
                return false;
            ...
            foreach ( var f in fieldNames )
                if ( obj.GetType ().GetFields ().Any ( x => x.Name == f ) )
                    _flag |= ( obj.GetType ().GetField ( f ).GetValue ( obj ) == null );
            foreach ( var f in fieldNames )
                if ( obj.GetType ().GetProperties ().Any ( x => x.Name == f ) )
                    _flag |= ( obj.GetType ().GetProperty ( f ).GetValue ( obj, null ) == null );
